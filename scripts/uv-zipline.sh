...
loop_validator_dir="${loop_repo_dir}/python/zipline_validation"
loop_python_version="$(tr -d '[:space:]' < "${loop_validator_dir}/.python-version")"

if [[ "${1:-}" != "run" ]]; then
  echo "uv-zipline.sh supports only isolated run; it never creates another .venv" >&2
  exit 2
fi
shift
cd "${loop_validator_dir}"
